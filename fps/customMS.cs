if($version !$= "") {
	error("This version of CustomMS is meant for The Orange Block 2.2");
	return;
}
...
function getBrickCount() {
	...
	for(%i=0;%i<MissionCleanup.getCount();%i++) {
		...
		if(%obj.getClassName() !$= "StaticShape") continue;
		if(%obj.getDataBlock().className !$= "brick") continue;
		if(strPos(strLwr(%obj.getSkinName()),"ghost") != -1) continue; // essentially isPlanted()
		%count++;
	}
	return %count;
}
