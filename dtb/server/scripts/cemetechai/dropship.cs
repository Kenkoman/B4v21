// Pelican Dropship -Edited from sigma fighter by DShiznit
...
datablock FlyingVehicleData(dropship)
{
	emap							= true;
	category						= "Vehicles";
	shapeFile						= "tbm/data/shapes/bricks/vehicles/tbmhuey2.dts";
   ...
   mountPointTransform[0] = "0 0 -1 0 0 1 0";
	...
	cameraMaxDist					= 50.0; // Why so much? So that 3rd person would look right, that's why!
	cameraOffset					= 15;
	cameraLag						= 5.0;
...
function servercmdpelican(%client) {
...
//  %block.mountImage(LPminigunimage,7);
...
function dropship::onTrigger(%data, %obj, %trigger, %state)
{
	if(%trigger == 0)
	{
...
function dropship::onCollision(%this,%obj,%col,%vec,%speed)
{
