...
function MessageHud_Edit::eval(%this)
{
   ...
   if(%text !$= "")
   {
      if(MessageHud.MsgType == 1)
         commandToServer('teamMessageSent', "\c5"@%text);
      if(MessageHud.MsgType == 2)
      commandtoserver('pmclient',Adminplayerlist.getselectedid(), "\c5"@%text);
      if(MessageHud.MsgType == 0)
...
function teamMessageHud(%make)
{
   if(%make)
   {
      MessageHud.MsgType = 1;
      MessageHud.toggleState();
   }
}
function pmMessageHud()
{
      Canvas.popdialog(Admingui);
      MessageHud.MsgType = 2;
      MessageHud.toggleState();
}
