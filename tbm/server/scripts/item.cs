...
//---------------------------------------------------------------
   ...
   //echo("item picked up");
	...
	if(%freeslot != -1) {
		%x = computePurchase(%user, %obj);
		if(!%x) return;
		%player.inventory[%freeslot] = %this;
		if(%this.image.skinnable)
			%player.weaponSkin[%this] = %obj.getSkinName(); //Note: %this in this case is a datablock ID, not a name
...
if(%player.getmountedimage(0) == nametoID(zombieimage) && %player.inventory[%invPosition] != nametoID(zombieweapon)) return;
	...
	%color = %player.weaponSkin[%this] $= "" ? %this.image.skinName : addTaggedString(%player.weaponSkin[%this]);
	...
	else
	{
		...
		if(%player.currWeaponSlot == %invPosition)
		{
			...
			if(strStr(strLwr(%player.getImageState(%mountPoint)), "reload") == -1) { //Make sure they're not reloading or something
				%player.unmountImage(%mountSlot);
				messageClient(%client, 'MsgHilightInv', '', -1);
				%player.currWeaponSlot = -1;
				//serverCmdBrickParse(%client, 1);
			}
			//Else means that this person is a cheater.`
		}
		else
		{
