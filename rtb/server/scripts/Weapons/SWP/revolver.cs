...
datablock ItemData(Revolver)
{
	...
	shapeFile = "~/data/shapes/Revolver.dts";
	skinName = 'grayDark';
	rotate = false;
	mass = 1;
	density = 0.2;
...
function Revolver::onUse(%this, %player, %InvPosition)
{
	//check for quiver 
	//if you dont have it, regular Revolver
	//if you do, super Revolver
	...
	if(%mountedImage)
	{
		if(%mountedImage == RevolverImage.getId() || %mountedImage == superRevolverImage.getId())
		{
			//some kind of Revolver mounted so, unmount it
		...
		{
			//something other than Revolver mounted, so do Revolver selection and mount
			if(%player.getMountedImage($BackSlot))
			{
				if(%player.getMountedImage($BackSlot) == quiverImage.getId())
				{
					%player.mountimage(superRevolverImage, $RightHandSlot, 1, %skin);
					messageClient(%client, 'MsgHilightInv', '', %InvPosition);
					%player.currWeaponSlot = %invPosition;
				}
				else
				{
					%player.mountimage(RevolverImage, $RightHandSlot, 1, %skin);
					messageClient(%client, 'MsgHilightInv', '', %InvPosition);
					%player.currWeaponSlot = %invPosition;
				}
			}
	...
	{
		//nothing mounted so do Revolver selection and mount
		//something other than Revolver mounted, so do Revolver selection and mount
		if(%player.getMountedImage($BackSlot))
		{
			if(%player.getMountedImage($BackSlot) == quiverImage.getId())
...
datablock ShapeBaseImageData(RevolverImage)
{
   ...
   skinName = 'brown';
   emap = true;
   PreviewFileName = "rtb/data/shapes/bricks/Previews/revolver.png";
	cloakable = false;
   ...
   // Initial start up state
	...
	stateSound[2]					= RevolverFireSound;
...
//Super Revolver- you get it from using quiver with Revolver
datablock ShapeBaseImageData(superRevolverImage)
{
   // Basic Item properties
   shapeFile = "~/data/shapes/Revolver.dts";
   skinName = 'brown';
   ...
   // Initial start up state
	...
	stateSound[4]					= RevolverFireSound;
...
function superRevolverImage::onCheck(%this,%obj,%slot)
{
	if(%obj.getMountedImage($BackSlot) !=  quiverImage.getId())
	{
		//we've somehow cheated so switch to regular Revolver
		//we have to do it this way because the image mount/unmount que system sucks
		//so its easy to cheat 

		%obj.mountimage(RevolverImage, $rightHandSlot);
	}
}

function bulletProjectile::onCollision(%this,%obj,%col,%fade,%pos,%normal) 
{ 
	weaponDamage(%obj,%col,%this,%pos,revolver);
	radiusDamage(%obj,%pos,%this.damageRadius,%this.radiusDamage,revolver,%this.areaImpulse);
}