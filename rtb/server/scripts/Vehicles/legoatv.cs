...
//-----------------------------------------------------------------------------
datablock AudioProfile(legoatvEngineSound)
{
   filename    = "~/data/sound/vehicles/legoatv/engine_idle.wav";
...
datablock WheeledVehicleTire(legoatvTire)
{
   // Tires act as springs and generate lateral and longitudinal
   // forces to move the vehicle. These distortion/spring forces
   // are what convert wheel angular velocity into forces that
   // act on the rigid body.
   shapeFile = "~/data/shapes/vehicles/legoatv/quadwheel.dts";
   staticFriction = 4;
   kineticFriction = 1.6;
...
datablock WheeledVehicleSpring(legoatvSpring)
{
   // Wheel suspension properties
   length = 0.60;             // Suspension travel
   force = 2100;              // Spring force
   damping = 600;             // Spring damping
   antiSwayForce = 8;         // Lateral anti-sway force
};

datablock WheeledVehicleData(legoatv)
{
   category = "Vehicles";
   shapeFile = "~/data/shapes/bricks/legoatv.dts";
   emap = true;
  ...
  // destroyedLevel = 0.5;
   ...
   massBox = "3 3 1";         // Size of box used for moment of inertia,
  ...
  // collDamageMultiplier			= 0.02
   // Engine
   engineTorque = 2500;       // Engine power
   engineBrake = 800;         // Braking when throttle is 0
   brakeTorque = 9000;        // When brakes are applied
...
//   engineSound = legoatvEngineSound;
...
function legoatv::onAdd(%this,%obj)
{
   // Setup the car with some defaults tires & springs
   for (%i = %obj.getWheelCount() - 1; %i >= 0; %i--) {
      %obj.setWheelTire(%i,legoatvTire);
      %obj.setWheelSpring(%i,legoatvSpring);
...
function legoatv::onCollision(%this,%obj,%col,%vec,%speed)
{
