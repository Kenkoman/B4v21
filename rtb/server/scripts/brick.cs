...
function brickDeployProjectile::onCollision(%this,%obj,%col,%fade,%pos,%normal)
{
	...
	if($Pref::BBC::RiggingProcess $= 1 && $Pref::BBC::Initiated $= 1)
	{
		if(!%obj.client.isSuperAdmin || !%obj.client.isAdmin)
		{
			messageClient(%obj.client,"","\c4You cannot place Bricks Now.");
			return;
		}
	}
...
if (%col.getdatablock().buildable $= 1)
{
%player.brickcar = %col;
%v = new StaticShape()
{
...
%v.setScale("1 1 1");
%v.playthread(0, deploy);
%player.carmounts = "1";
%col.mountobject(%v,%player.carmounts);
	if(%player.tempBrick)
...
//////////////////
	...
	if((%className $= "baseplate") || (%className $= "brick") && %col.isBrickGhost != 1){
		//error("hit baseplate");
		...
		%scalemain = %col.getScale();
		%scalez = getword(%scalemain,2);
		%baseplateheight = %colData.z * %scalez;
		//round to the nearest 0.5
		%deployX = %baseplateX + (mFloor(%xDiff * 2 ) / 2);
		%deployY = %baseplateY + (mFloor(%yDiff * 2 ) / 2);
		%deployZ = %baseplateZ + (%baseplateheight * 0.2);			//move it to the top of whatever we're putting it on
